...
const PRODUCTS = [
  ...
  { title: "Sentence Strips (100 pack)", price: "Usually under $8", store: "Amazon", featured: false, link: `https://www.amazon.com/s?k=sentence+strips+classroom&tag=${AMAZON_TAG}` },
  ...
  { title: "Teacher Caddy Organizer", price: "Usually $12–$18", store: "Amazon", featured: false, link: `https://www.amazon.com/s?k=teacher+caddy+organizer&tag=${AMAZON_TAG}` },
...
app.get("/", (req, res) => {
  ...
  res.send(`
    <div style="font-family:Segoe UI,Arial;background:#f8faf7;padding:60px;text-align:center;">
      ...
      <p style="max-width:720px;margin:10px auto 18px;color:#555;font-size:15px;">
        Prices shown are typical classroom price ranges. Final prices may vary by brand and availability.
      </p>

      <p style="max-width:720px;margin:0 auto 30px;color:#555;font-size:14px;">
        <strong>Affiliate Disclosure:</strong> As an Amazon Associate, I earn from qualifying purchases.
      </p>

      ${filtered.map(p => `
        <div style="display:inline-block;width:280px;margin:16px;padding:22px;background:white;border-radius:18px;box-shadow:0 10px 25px rgba(0,0,0,.08);vertical-align:top;">
          ${storeBadge(p.store)}<br><br>
          <strong>${p.title}</strong><br><br>
          <span style="color:#2e7d32;font-size:18px;font-weight:bold;">${p.price}</span><br><br>
